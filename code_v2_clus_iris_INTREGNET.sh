...
in_dir=/home/users/mali/Tasks/core_networks/net_oct_2019/$1/
out_dir=/home/users/mali/Tasks/core_networks/net_oct_2019/$1/
K27ac='_K27ac'
K4me3='_K4me3'
mRNA='_mRNA'
DNaseFile='_DNase'
...
awk 'FNR==NR {a[$1]; next} ($(NF-1) in a)' ${in_dir}$i /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Processed_GeneHancer_GRCh38.bed > "${name}_Enhancers.tmp"
...
# -F 1.0
bedtools intersect -a "sorted_${name}_Act_Enhancers.tmp" -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Cistrome_ChIPseq_sorted.bed -F 1.0 -wo -sorted> "${name}_All_TFs_In_Core_Enh.tmp"
#bedtools intersect -a "sorted_${name}_Act_Enhancers.tmp" -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/MN_Chip_PWM_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_Core_Enh.tmp"
...
awk 'FNR==NR {a[$1]; next} ($(NF-1) in a)' NC_Uniq_TFs_Binding_CoreEnh.tmp /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Processed_GeneHancer_GRCh38.bed > NC_TFs_Enh.tmp
...
bedtools intersect -a Filtered_NC_TFs_Act_Enh_Sorted.tmp -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Cistrome_ChIPseq_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_NonCore_Enh.tmp"
#bedtools intersect -a Filtered_NC_TFs_Act_Enh_Sorted.tmp -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/MN_Chip_PWM_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_NonCore_Enh.tmp"
...
#bedtools intersect -a "${name}_Shell1_Enh.bed" -b ${in_dir}$DNase -wa > "${name}_Shell1_Enh_Acc.bed"
# remove non-TF rows from the bed file
awk 'FNR==NR {a[$1]; next} ($(NF-2) in a)' /home/users/mali/Tasks/core_networks/Evan_networks/input_files/AnimalTFDB_TF.bed "${name}_Shell1_Enh_Acc.bed" > "${name}_Shell1_Enh_Acc_OnlyTF.tmp"
awk 'FNR==NR {a[$1]; next} ($(NF-10) in a)' /home/users/mali/Tasks/core_networks/Evan_networks/input_files/AnimalTFDB_TF.bed "${name}_Shell1_Enh_Acc_OnlyTF.tmp" > "${name}_Shell1_Enh_Acc_OnlyTF.bed"
...
#: <<'end_long_comment'
awk 'FNR==NR {a[$1]; next} ($(NF-2) in a)' ${in_dir}$i /home/users/mali/Tasks/core_networks/Evan_networks/input_files/GRCh38_promoter.bed > "${name}_Promoters.tmp"
wc -l "${name}_Promoters.tmp"
sort -k1,1 -k2,2n "${name}_Promoters.tmp" > "sorted_${name}_Promoters.tmp"
wc -l "sorted_${name}_Promoters.tmp"
cp "sorted_${name}_Promoters.tmp" "sorted_${name}_Promoters.txt"
...
cp "${name}_Act_Pro.tmp" "${name}_Act_Pro.txt"

# -F 1.0 means entire ChIP-peak should fall within Active promoter region
# -F 1.0 -wo ..
bedtools intersect -a "${name}_Act_Pro.tmp" -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Cistrome_ChIPseq_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_Core_Pro.tmp"
#bedtools intersect -a "${name}_Act_Pro.tmp" -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/MN_Chip_PWM_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_Core_Pro.tmp"
...
awk 'FNR==NR {a[$1]; next} ($(NF-2) in a)' NC_Uniq_TF_Binding_CorePro.tmp /home/users/mali/Tasks/core_networks/Evan_networks/input_files/GRCh38_promoter.bed > NC_TFs_Promoters.tmp
...
# -F 1.0 -wo ..
bedtools intersect -a NC_TFs_Act_Pro.tmp -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/Cistrome_ChIPseq_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_NonCore_Pro.tmp"
#bedtools intersect -a NC_TFs_Act_Pro.tmp -b /home/users/mali/Tasks/core_networks/Evan_networks/input_files/MN_Chip_PWM_sorted.bed -F 1.0 -wo -sorted > "${name}_All_TFs_In_NonCore_Pro.tmp"
...
#bedtools intersect -a "${name}_Shell1_Pro.bed" -b ${in_dir}$DNase -wa > "${name}_Shell1_Pro_Acc.bed"
...
# remove for only the TF genes (originally I also had CRM and co-factors in TF gene list)
awk 'FNR==NR {a[$1]; next} ($(NF-3) in a)' /home/users/mali/Tasks/core_networks/Evan_networks/input_files/AnimalTFDB_TF.bed "${name}_Shell1_Pro_Acc.bed" > "${name}_Shell1_Pro_Acc_OnlyTF.tmp"
awk 'FNR==NR {a[$1]; next} ($(NF-8) in a)' /home/users/mali/Tasks/core_networks/Evan_networks/input_files/AnimalTFDB_TF.bed "${name}_Shell1_Pro_Acc_OnlyTF.tmp" > "${name}_Shell1_Pro_Acc_OnlyTF.bed"
